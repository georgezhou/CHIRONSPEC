...
def make_spectrum(spectrum,teff_init):
    ...
    teff_init = int(roundnearest(teff_init,250))
...
def runML(spectrum):
    ...
    teff_init = 5000
    spec = make_spectrum(spectrum,teff_init)

    
    isgiant = pickle.load(open("SPEC_ML/isgiant.pkl","rb"))
    classification = isgiant.predict([spec[:-600]])
    ...
    if classification[0] == 0:
        mor = pickle.load(open("SPEC_ML/mor_ms.pkl","rb"))
    if classification[0] == 1:
        mor = pickle.load(open("SPEC_ML/mor_giants.pkl","rb"))
    ...
    ###### Now run that all again with a new correct teff template for normalisation
    spec = make_spectrum(spectrum,teff)
    ...
    if classification[0] == 0:        
        plt.text(0.1,0.9,"Dwarf",color="k",fontsize=15, weight="black",ha="left",transform=ax.transAxes)
    if classification[0] == 1:        
        plt.text(0.1,0.9,"Giant",color="r",fontsize=15, weight="black",ha="left",transform=ax.transAxes)
...
def runall():
    ...
    #for i in range(5):
        ...
        if os.path.exists(fits):
            ...
            database.loc[i]["teff"] = teff
            database.loc[i]["logg"] = logg
            database.loc[i]["feh"] = feh
